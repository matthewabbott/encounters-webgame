...
## Consumables: Exploits

### Theme: Vulnerabilities & Exploits

By completing encounters (or finding them on map), you **discover vulnerabilities** in the system. Through these vulnerabilities, you can **run exploits** to achieve specific effects. Each exploit is consumable - once used, the system patches the vulnerability.

**Narrative**: You're a netrunner finding and exploiting security holes before they're patched.

**Exploit Hierarchy**:
- **Exploits** (umbrella term for all consumables)
  - **Commands** (simple shell commands - current implementation)
  - **Scripts** (more complex, multi-step effects - future)
  - **Programs** (most powerful, advanced effects - future)

### Currently Implemented: Commands (Simple Exploits)

**Commands** are primitive, single-operation exploits. They're simple shell commands that manipulate your deck.

#### Deck Manipulation Commands (Permanent Effects)
- `fork` - Copy a card permanently (Unix fork() system call)
- `shred` - Destroy a card permanently (secure file deletion)
- `++` - Change card value by ±1 permanently (increment/decrement operator)

### Future: Scripts (More Complex Exploits)

Scripts would be more sophisticated multi-step operations:

#### 1. Deck Manipulation Scripts
- `duplicate.bat` - Copy multiple cards
- `stack.py` - Arrange top X cards in chosen order
- `mark.sh` - "Mark" cards so you know when they're coming
- `chmod` - Change card suit/permissions
...
- `grep` - Search and extract specific cards

#### 3. Meta Scripts (Modifiers)
- `sudo` - **Massively powerful one-time effect** (elevated privileges)
- `overload.bat` - **Next exploit affects ALL cards in hand**
  - `overload` + `fork` = copy entire hand
  - `overload` + `shred` = destroy entire hand
  - `overload` + `++` = boost all cards by +1
  - Incredibly powerful combo system!

### Future: Programs (Most Powerful Exploits)

Full programs would be the most advanced tier:

#### World Programs (Map Interaction)
...
- `tar` - Archive/bundle multiple cards
- `cat` - Concatenate cards together

**Implementation Priority**:
- ✅ Phase 1: Commands (`fork`, `shred`, `++`)
- Phase 2: More commands (`chmod`, `grep`, `sudo`)
- Phase 3: Scripts (multi-step operations)
- Phase 4: Programs (map interaction, advanced effects)
...
Exploits work BOTH in encounters AND on world map:
- `shred` destroys card in encounter OR destroys barrier on map
- `++` changes card value OR changes lock combination
